...
void *memchr_opt(const void *src_void, int c, size_t length)
{
    ...
    if (!TOO_SMALL(length)) {
        ...
        while (length >= LBLOCKSIZE) {
            ...
            // if the first block contain the search character, stop search
            if (DETECT_CHAR(*asrc, mask))
                break;
            // if not, move to the next block
            asrc++;
            // subtract the number of char contained in a block
            length -= LBLOCKSIZE;
        }
...
int main()
{
    ...
    char *ret = (char *)memchr_opt(str, ch, strlen(str));
